...
class OrderRepository {
  ...
  Future insertOrder(OrderEntity order) async {
    final searchedOrder = await dao.findOrderByType(order.date, order.type);
    if (searchedOrder == null) {
      await dao.insertOrder(order);
      return;
    }
    await dao.addSameDate(order.date, order.type, order.weight, order.deposit);
  }
  ...
  Future<List<DayOrder>> getMonthOrders(int year, int month) async {
    ...
    var orders = await dao.getOrderDate(start, formatter.format(end));
    return _convertEntityToOrder(orders, year, month);
  }

  Future<List<MonthOrder>> getYearOrders(int year) async {
    var orders = await dao.getOrderDate('$year-01-01', '$year-12-31');
